...
//追加元素
var sponsor_html = '<input id="sponsor" type="checkbox"><dialog class="box sponsor" open><header class="title">赞助任意金额</header><section><p>长按识别或扫描收款码</p><img src="https://cdn.jsdelivr.net/gh/lwq057/cdn@1.5/mc.k7dj.com/sponsor.png" alt="二维码"><p>牛逼喊麦网致力于中国网络Mc喊麦文化传播发展</p><p>感谢有你们的支持 让我们更好的前行</p></section><footer><label for="sponsor">确定</label></footer></dialog>';
...
function playlink(id,name){
    if (!id){
        return '<a href="#/">牛逼喊麦网</a>';
    }
    if (name){
        return '<a data-id="'+id+'" href="#/mc_'+id+'">'+name+'</a>';
    }
    return window.location.protocol+"//"+window.location.host+'/mc_'+id;
}
...
function player(){
    //if (play = document.getElementsByClassName('play')[0]){
        ...
        radios = function (){
            if (radio = document.getElementById('radio')){
                ...
                if (ids){
                    ...
                    if (data['radio']){
                        radio_url = '/radio_'+data['radio'];
                        if (radio_page){
                            radio_url += '_'+radio_page;
                        }
                    }else{
                        radio_url = '/radio';
                        if (radio_page){
                            radio_url += '__'+radio_page;
                        }
                    }
                }
            }
        ...
        list_play = function (i){
            ...
            if (document.getElementById('radio')){
                play_cover.getElementsByTagName('img')[0].src = '/plugin_image_'+playlist[i].name+'.jpg';
            }
            if (play_down){
...
if ( Object.keys(players.playlist).length == 0 ){
    add_playlist({id:'1',name:'牛逼喊麦网',url:'/static/nb.mp3'});
}
// setTimeout(function(){
    // play_playlist( getCookie('audio_id') || 1 );
// },1000);
...
function share(music_id,music_name){
    ...
    if ( share_box = document.querySelector('dialog.share')){
        ...
        var img = window.location.origin+'/plugin_image_'+name+'.jpg';
        var txt = name+'在线试听下载';
        var site = '牛逼喊麦网';

        var html = '';
		html += '<a href="http://service.weibo.com/share/share.php?url='+url+'&title='+ encodeURIComponent("#喊麦# "+txt) +'&pic='+img+'" target="_blank">新浪微博</a>';
		...
		html += '<a href="https://twitter.com/intent/tweet?url='+encodeURIComponent(url)+'&text='+encodeURIComponent(txt)+'&pic='+encodeURIComponent(img)+'&hashtags=mc" target="_blank">Twitter</a>';
