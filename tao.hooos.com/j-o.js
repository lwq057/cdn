            ...
            if (data.props.groupProps && data.props.groupProps != 'undefined'){
                $.each(data.props.groupProps[0], function (k, v) {
                    $.each(v, function (kk, vv) {
                        ...
                        attr += '<li title="' + vv[kkk] + '">' + kkk + ': ' + vv[kkk] + '</li>';
                    });
                });
            }
            if (data.props.importantProps) {
                $.each(data.props.importantProps, function (k, v) {
                    attr += '<li title="' + v['value'] + '">' + v['name'] + ': ' + v['value'] + '</li>';
                });
            }
            if (JSON.stringify(data.props2) != "{}") {
                if (typeof (data.props2.groupProps) != 'undefined') {
                    $.each(data.props2.groupProps[0], function (k, v) {
                        $.each(v, function (kk, vv) {
                            var kkk = Object.keys(vv)[0];
                            attr += '<li title="' + vv[kkk] + '">' + kkk + ': ' + vv[kkk] + '</li>';
                        });
                ...
                if (data.props2.importantProps) {
                    $.each(data.props2.importantProps, function (k, v) {
...
function get_desc() {
    ...
    if (id) {
        if ($('body>main>section>div>div[n]').length > 0) {
            var api = 'https://hws.m.taobao.com/cache/desc/5.0?id=' + id;
            $.ajax({
                url: api,
                timeout: 1000,
                tryCount: 0,
                ...
                success: function (result) {
                    this.tryCount++;
                    if (result.wdescContent && result.wdescContent.pages) {
                        var regx = /<[^>]*>|<\/[^>]*>/gm;
                        var desc_html = '';
                        $.each(result.wdescContent.pages, function (i, v) {
                            if (v.indexOf("</img>") != -1) {
                                var src = v.replace(regx, '');
                                desc_html += '<img alt="' + document.title + ' - 详情图" data-original-url="' + src + '" src="' + src + '">';
                            } else if (v.indexOf("</txt>") != -1) {
                                desc_html += "<p>" + v.replace(regx, '') + "</p>";
                            }
                        });
                        descattr({ desc: desc_html });
...
function get_attr() {
    ...
    if (id) {
        if ($('body>main>section>ol[n]').length > 0) {
            var api = 'https://acs.m.taobao.com/h5/mtop.taobao.detail.getdetail/6.0/?data=%7B"itemNumId"%3A"' + id + '"%7D';
            $.ajax({
                url: api,
                timeout: 1000,
                tryCount: 0,
...
function load_viewer() {
    if ($('body>main>article>div[imgs]>figure').length > 0) {
        if (typeof (window.figure_viewers) !== 'object') {
            window.figure_viewers = new Viewer($('body>main>article>div[imgs]>figure')[0], {
                url: 'data-original-url', shown: function () {
                    lazyload_run();
                }, view: function (o) {
    ...
    if ($('body>main>section>div>div').length > 0) {
        if (typeof (window.viewers) !== 'object') {
            window.viewers = new Viewer($('body>main>section>div>div')[0], {
                url: 'data-original-url', shown: function () {
                    lazyload_run();
                }
            });
