...
function datas_list(data,type){
    ...
    switch (type) {
        ...
        case 'shop':
            ...
            break;
        case 'activity':
            let tags = ['','大促活动','人气活动','常规活动'];
            let status = ['','即将开始','进行中','已结束'];
            $.each(data,function(i,v){
                html += '<li>';
                html += '<a href="/go/?aurl='+((v.platformType == 2) ? v.urlM : v.urlPC )+'">';
                    html += '<i>';
                    if (v.hasOwnProperty('imgList') && (v.imgList.length > 0)){
                        let imgwh = [];
                        $.each(v.imgList,function(ii,vv){
                            imgwh[vv.widthHeight] = ii;
                        });
                        let imgi = 0;
                        if (imgwh['350*350']){
                            imgi = imgwh['350*350'];
                        }else if (imgwh['250*250']){
                            imgi = imgwh['250*250'];
                        }
                    ...
                    html += '</i>';
                    html += '<em>';
                        html += '<i>'+status[v.actStatus]+'</i>';
                        if (v.hasOwnProperty('recommend')){
                            html += '<i>'+v.recommend+'星推荐</i>';
                        }
                    html += '</em>';
                    html += '<h4>'+v.title+'（'+v.advantage+'）</h4>';
                    html += '<p>';
                        if (v.actStatus == 1){
                    ...
                    html += '</p>';
                html += '</a>';
                html += '<a '+((v.actStatus==2) ? 'c' : '')+' href="/go/?aurl='+((v.platformType == 2) ? v.urlM : v.urlPC )+'">'+status[v.actStatus]+'</a>';
                html += '</li>';
            });
            break;
        default:
            $.each(data,function(i,v){
                html += '<li>';
                html += '<a href="/g-'+v.skuId+'/">';
                    html += '<i>';
                    html += '<img src="'+v.imageInfo.imageList[0].url+'">';
                    if (v.hasOwnProperty('documentInfo')){
                        html += '<q>优惠 '+v.documentInfo.document+'</q>';
                    }else{
                        html += '<u>'+v.brandName+' ';
                            if (v.owner == 'g'){
                                html += '京东自营 ';
...
$('body>main>article').ready(function(){
    ...
    if ($('body>main>article label[for="share"]').length>0){
        ...
        var tag = s['tag'] = $('body>main>article>div[info]>div[t]>i').map(function(){
            return $(this).text();
        }).get().join('、') || '';
...
$('body>main>article').ready(function(){
    var id = $('input[name="id"]').val();
    if (id){
        var api = 'https://wqsitem.jd.com/detail/'+id+'_d_normal.html';
        $.ajax({
            url:api,
            timeout:1000,
            tryCount:0,
