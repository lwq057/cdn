...
}$("body>main>section>div>div").append($("main>article figure").html());load_viewer();
...
s.tag=$("body>main>article>div[info]>div[t]>i").map(function(){return $(this).text()
}).get().join("、")||"";s.cat=$("body>main>div>nav>a").eq(0).text()||"";var img_src="/server/share.html?";
s.title=encodeURI(s.title);$.each(s,function(i,v){img_src+="&"+i+"="+v});s.title=title;
s.num=$("body>main>article>div[info]>b>i").map(function(){return $(this).text()}).get().join("、")||"";
var s_t='<img src="'+s.img+'"><br>';if(s.cat){s_t+="#"+s.cat+"#<br>"}s_t+=s.title+"<br>";
if(s.num){s_t+=s.num+"<br>"}if(s.oprice){s_t+="【原售价】"+s.oprice+"￥"}if(s.price){if(s.coupon){s_t+="【在售价】"+s.price+"￥<br>"
}else{s_t+="【优惠价】"+s.price+"￥<br>"}}if(s.coupon){s_t+="【券后价】"+(s.price-s.coupon).toFixed(2)+"￥";
s_t+="【优惠券】"+s.coupon+"￥<br>"}if(s.tag){s_t+="[ "+s.tag+" ]<br>"}s_t+="购买链接："+s.url+"<br>";
...
}});function load_viewer(){if($("body>main>article>div[imgs]>figure").length>0){if(typeof(window.figure_viewers)!=="object"){window.figure_viewers=new Viewer($("body>main>article>div[imgs]>figure")[0],{url:"data-original-url",shown:function(){lazyload_run()
...
if(document.referrer.indexOf(document.domain)==-1){$(window).bind("beforeunload",function(){if(window.is_confirm!==false){return"天猫淘宝的优惠券商品都在虎窝淘"
